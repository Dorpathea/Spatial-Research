...
class Dataset_Spatial(Dataset):
    ...
    def __getitem__(self, index):
        while (1):
            ...
            if (index >= len(self.data_x) - self.in_len - 1) or (
                    index >= len(self.data_y) - self.in_len - self.out_len - 1):
                index = 0
            ...
            if (self.data_x[s_begin, -1] != self.data_x[s_end, -1]) or (
...
class Exp_crossformer(Exp_Basic):
    ...
    def test(self, setting, save_pred=False, inverse=False):
        ...
        if (save_pred):
            ...
            #np.save('./Prediction_Results/' + args.setting_name + '/' + 'pred.npy', preds)
            #np.save('./Prediction_Results/' + args.setting_name + '/' + 'true.npy', trues)
