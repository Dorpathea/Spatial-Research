...
class Dataset_Spatial(Dataset):
    ...
    def __read_data__(self):
        ...
        # Moving lat and lon to last column
        data = np.roll(data, -2, 1)
    ...
    def __getitem__(self, index):
      ...
      # Added noise to reduce overfitting in training loss
      if self.flag == 'train':
          myNoise = np.random.normal(loc=0, scale= 1, size=seq_x.shape)
          seq_x = seq_x + (0.1 * myNoise)
...
class SpatialEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x[:, :, -2:]
        # Do linear embedding with ReLU
        label = self.relu(self.embed(x))

        # Do linear embedding with ReLU
        label2 = self.relu(self.embed2(label))
...
class Exp_crossformer(Exp_Basic):
    ...
    def _select_optimizer(self):
        # Edited to add in l2 with weight_decay equal to the learning rate --> CHANGED
    ...
    def vali(self, vali_data, vali_loader, criterion):
        ...
        print('vali mse:{}, mae:{}'.format(mse, mae))
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            metrics_mean = metrics_all.sum(axis=0) / instance_num
            mae, mse, rmse, mape, mspe = metrics_mean
            print('train mse:{}, mae:{}'.format(mse, mae))
