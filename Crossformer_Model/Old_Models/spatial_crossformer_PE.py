...
class Dataset_Spatial(Dataset):
    ...
    def __read_data__(self):
        ...
        df_data = df_label[df_label.columns[2:]]          # Excluding date and label
        df_data_labels = df_label[df_label.columns[1:]]   # Excluding date

        # Scale the data only, excluding date and label
        if self.scale:
            if self.scale_statistic is None:
                self.scaler = StandardScaler()
                train_data = df_data[border1s[0]:border2s[0]]
...
class SpatialEmbedding(nn.Module):
    def __init__(self, seg_len, d_model, max_len=5000):
        ...
        #   print('PositionalEmbedding: d_model = ', d_model)
